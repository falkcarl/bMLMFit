...
# Set constants
J      = c(5,15,45)   # cluster sizes to try (level 2)
N      = c(5,15,45)   # observation levels to try (level 1)
G00    = 1            # fixed intercept
TAU0   = .09          # random intercept variance
G10    = .3           # fixed slope of X1
TAU1   = .09          # random slope of X1
G20    = .3           # fixed slope of X2
TAU2   = 0            # random slope of X2
G30    = 0            # fixed slope of X3
TAU3   = 0            # random slope of X3
RHO01  = 0            # corr between TAU0 and TAU1
...
R_12   = 0.4          # correlation between X1 and X2
R_13   = 0.4          # correlation between X1 and X3
R_23   = 0.4          # correlation between X2 and X3
TAUX   = 0.1          # level 2 cluster variance for predictors
SIGMAX = 0.9          # residual variance for predictors
...
conds = expand.grid(N=N, J=J,G00=G00,TAU0=TAU0,G10=G10,
                    ...
                    X2_sd=X2_sd,X3_sd=X3_sd,R_12=R_12,R_13=R_13,R_23=R_23,
                    TAUX=TAUX, SIGMAX=SIGMAX,
                    S=S,nchain=nchain,niter=niter,burn=burn,thin=thin,
                    marg=marg)
