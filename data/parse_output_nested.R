...
for(f in rds_files) {
  ...
  for(i in 1:length(x)) {
    ...
    g0s = sapply(x[[i]]$fixef, function(j) j['Intercept','Estimate'])
    g1s = sapply(x[[i]]$fixef, function(j) j['X1','Estimate'])
    g2s = sapply(x[[i]]$fixef, function(j) {
      if('X2' %in% rownames(j)) return(j['X2','Estimate'])
      return(NA)
    })
    ...
    # mean taus per model
    t0s = sapply(x[[i]]$ranef, function(j) j$g$sd['Intercept','Estimate'])
    ...
    # difference from true value
    d_t0s = conds_x[['TAU0']] - t0s^2
    d_t1s = conds_x[['TAU1']] - t1s^2
