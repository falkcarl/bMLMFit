...
hist(all_rhats[1,], col = MODEL_COLS[1], xlim=c(.99,2), 
     xlab=expression('Max'~hat(R)), main='', ylim=c(0,4000))
...
### cond
hist(rhats_cond[1,], col = MODEL_COLS[1], xlim=c(.99,2), 
     xlab=expression('Max'~hat(R)), ylim=c(0,1000), 
...
hist(rhats_marg[1,], col = MODEL_COLS[1], xlim=c(.99,1.05), 
     xlab=expression('Max'~hat(R)), ylim=c(0,200), 
...
for(n in unique(dat$N)) {
  for(j in unique(dat$J)) {
    rhats_cond = sapply(dat[dat$marg==0 & dat$N==n & dat$J==j,'max_rhats'], function(x) x)
    rhats_marg = sapply(dat[dat$marg==1 & dat$N==n & dat$J==j,'max_rhats'], function(x) x)
    all_rhats  = cbind(rhats_cond, rhats_marg)
    rhat_p   = mean(all_rhats > 1.05)
    
    pdf(paste0('figs/diag/rhats_hists_n',n,'_j',j,'.pdf'), 12, 6)
    layout(matrix(1:2,1,2))
    par(mar=c(5, 4, 8, 2) + 0.1)
    
    ### cond
    hist(rhats_cond[1,], col = MODEL_COLS[1], xlim=c(.99,2), 
         xlab=expression('Max'~hat(R)), ylim=c(0,100), 
         main="Conditional Estimation")
    for(i in 2:nrow(rhats_cond)) hist(rhats_cond[i,], col=MODEL_COLS[i], add=T)
    abline(v=1.05, lty=2)
    legend('topright', bty='n', fill=MODEL_COLS, 
           legend=LETTERS[1:nrow(rhats_cond)], title='Model', 
           ncol=2)
    legend('right',bty='n', legend=paste0('P(>1.05) = ',round(mean(rhats_cond > 1.05),4)))
    ...
    for(i in 2:nrow(rhats_marg)) hist(rhats_marg[i,], col=MODEL_COLS[i], add=T)
    abline(v=1.05, lty=2)
    legend('right',bty='n', legend=paste0('P(>1.05) = ',round(mean(rhats_marg > 1.05),4)))
    
    
    ### title + save
    mtext(paste0("N = ", n, ", J = ", j), side=3, line=-2, outer = TRUE, cex = 2)
    dev.off()
  }
}
...
hist(ess_all[1,], col = MODEL_COLS[1], breaks = 10,
     xlab = 'Min ESS', main='', ylim=c(0,1000), xlim=c(0,6000))
...
hist(ess_cond[1,], col = MODEL_COLS[1], breaks = 10,
     xlab = 'Min ESS', main='Conditional Estimation', ylim=c(0,500), xlim=c(0,6000))
...
hist(ess_marg[1,], col = MODEL_COLS[1], breaks = 10,
     xlab = 'Min ESS', main='Marginal Estimation', ylim=c(0,500), xlim=c(0,6000))
